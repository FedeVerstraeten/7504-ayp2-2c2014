...
\section{Métodos de la Transformada}
  ...
  \subsection{FFT}
   ...
   f(N) &= N
    ...
    Al principio, todas las sentencias son de orden constante hasta que
    aparecen el primer ciclo de N iteraciones. Dentro de este hay otro cilco de N 
...
calculate_dft_generic(Vector<Complex> const &x, bool inverse)
{
  ...
  return X;
    ...
    \end{lstlisting}
	Analizamos el coste temporal ---$T(p)$--- de la función (\textit{pow\_Complex}):
	\begin{lstlisting}
		Complex
    ...
    \end{lstlisting}
	  ...
	  a la mitad, tenemos 1 llamada de coste $T(p/2)$
