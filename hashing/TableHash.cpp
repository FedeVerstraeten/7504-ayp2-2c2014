...
void TableHash :: insert(const TypeElement &el)
{
    ...
    position=hashFunction(el.getKey());
    p=searchNode(el.getKey());
...
void TableHash :: remove(const string clave)
{
    ...
    if(table[position]!=NULL) // No esta vacia
    {
        ...
        while((actual->getNext()!=NULL) && (actual->getElement()).getKey()!=clave)
        {
            anterior=actual;
            actual=actual->getNext();
        }

        if((actual->getElement()).getKey()!=clave)
...
void TableHash :: print (const TypeElement &el)
{
    NodeTable *auxNode=NULL;

    auxNode=searchNode(el.getKey());
