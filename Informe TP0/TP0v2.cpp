//Charlie:
//PENDIENTE: Probar distintos archivos de texto para tener un minimo de robusticidad.
//PENDIENTE: La funcion impresion printElements() debe imprimir por flujo de salida en vez de por pantalla-
//Cristian:
//PENDIENTE: Validar la cantidad de conexiones respecto de la cantidad de NetworkElementes.
//Pensar una logica de arbol para validar cantidad de conexiones.
...
int main(void)
{
    ...
    cout<<endl<<"total de: "<<n<<"lineas."<<endl<<endl; // esta linea es un flag para ver si n se computo bien.
    ...
    printNetworkName((*lines_array[0]),cout); // esta tiene el contenido del nombre de la red
    for(i=1;i<n;i++)    processLine((*lines_array[i]));
    printElements(number_of_elements, cout);     //esta funcion printElements() es la que se encarga de la salida por pantalla.
